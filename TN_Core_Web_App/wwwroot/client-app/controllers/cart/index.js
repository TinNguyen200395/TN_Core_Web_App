    ...
    function registerEvents() {
        $('body').on('click', '.btn-delete', function (e) {
            ...
            $.ajax({
                ...
                success: function () {
                    tn.notify('Removing product is successful.', 'success');
                    loadHeaderCart();
                    loadData();
                }
            });
        ...
        $('body').on('keyup', '.txtQuantity', function (e) {
            ...
            if (q > 0) {
                $.ajax({
                    ...
                    success: function () {
                        tn.notify('Update quantity is successful', 'success');
                        loadHeaderCart();
                        loadData();
                    }
                });
            } else {
                tn.notify('Your quantity is invalid', 'error');
            }
        ...
        $('#btnClearAll').on('click', function (e) {
            ...
            $.ajax({
                ...
                type: 'post',
                success: function () {
                    tn.notify('Clear cart is successful', 'success');
                    loadHeaderCart();
                    loadData();
                }
            });
                    ...
                    render += Mustache.render(template,
                        {
                            ...
                            Amount: tn.formatNumber(item.Price * item.Quantity, 0),
                            Url: '/' + item.Product.SeoAlias + "-p." + item.Product.Id + ".html"
                        });
                    totalAmount += item.Price * item.Quantity;
                });
                $('#lblTotalAmount').text(tn.formatNumber(totalAmount, 0));
                if (render !== "")
